...
class UART_Port:
    ...
    def send_data(self, led_data):
        ...
        if led_data.rb_var.get() == 0:
            ...
            self.Serial_Port.write(bytes([led_data.green_val_1]))
            self.Serial_Port.write(bytes([led_data.blue_val_1]))
        else:  
            self.Serial_Port.write(bytes([254]))
            self.Serial_Port.write(bytes([led_data.red_val_2]))
            self.Serial_Port.write(bytes([led_data.green_val_2]))
            self.Serial_Port.write(bytes([led_data.blue_val_2]))
